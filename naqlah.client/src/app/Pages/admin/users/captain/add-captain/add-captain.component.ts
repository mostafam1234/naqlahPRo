...
import { DeliveryManAdminClient, VehicleAdminClient, VehicleTypeDto } from 'src/app/Core/services/NaqlahClient';
...
export class AddCaptainComponent implements OnInit, OnDestroy {
  ...
  deliveryTypes = [
    {
      ...
      label: 'مقيم',
      description: 'كابتن مقيم في المملكة',
      icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'
    },
    {
      value: DeliveryType.Citizen,
      label: 'مواطن',
      description: 'كابتن سعودي الجنسية',
  ...
  vehicleOwnerTypes = [
    { value: VehicleOwnerType.Resident, label: 'مالك فرد' },
    { value: VehicleOwnerType.Company, label: 'شركة / مؤسسة' },
    { value: VehicleOwnerType.Renter, label: 'مستأجر' }
  ];

  wizardSteps: WizardStep[] = [
    {
      key: 'account',
      title: 'الحساب والبيانات الأساسية',
      subtitle: 'معلومات الدخول والتعريف الشخصي',
      fields: ['email', 'password', 'fullName', 'phoneNumber', 'identityNumber', 'birthDate', 'deliveryType', 'deliveryLicenseType']
    },
    {
      key: 'vehicle',
      title: 'بيانات المركبة والمالك',
      subtitle: 'معلومات المركبة ونوع مالكها',
      fields: ['vehicleTypeId', 'vehicleBrandId', 'vehiclePlateNumber', 'vehicleOwnerTypeId', 'vehicleOwnerName']
    },
    {
      key: 'documents',
      title: 'المستندات والمرفقات',
      subtitle: 'جميع صور الكابتن والمركبة والمالك في مكان واحد',
      fields: []
    }
  ];
  ...
  constructor(
    ...
    this.captainForm = buildCaptainForm(this.fb, false);
  ...
  nextStep(): void {
    if (!this.validateStep(this.currentStep)) {
      this.toasterService.error('بيانات ناقصة', 'يرجى تعبئة الحقول المطلوبة في هذه الخطوة');
      return;
    }
  ...
  onSubmit(): void {
    ...
    const body = formValueToApiDto(this.captainForm.getRawValue(), this.datePipe, false);

    this.subs.sink = this.deliveryManClient.addDeliveryMan(body).subscribe({
      next: () => {
        this.isSubmitting = false;
        this.toasterService.success('تمت العملية بنجاح', 'تم إضافة الكابتن الجديد بنجاح');
        setTimeout(() => this.router.navigate(['/admin/users/captains']), 1500);
      },
      error: (error) => {
        this.isSubmitting = false;
        this.toasterService.error('خطأ', error?.errorMessage || 'حدث خطأ أثناء إضافة الكابتن');
      }
    });
  }
